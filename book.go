...
func getAllBooks(query string, limit, offset int) ([]Book, error) {
	//Retrieve
	books := []Book{}
	...
	for rows.Next() {
		...
		err = rows.Scan(&id, &name, &author, &pages, &publicationDate, &genre, &onSale, &price)
		if err != nil {
			return books, err
		}
